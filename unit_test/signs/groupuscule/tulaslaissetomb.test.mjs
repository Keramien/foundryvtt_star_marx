...
const SIGN = catalog.find(entry => entry.slug === "tulaslaissetomb");

describe("Kamarade Sign - Tu l'as laisse tomber par terre (tulaslaissetomb)", () => {
  test("catalog entry is available", () => {
    assert.ok(SIGN);
    assert.equal(SIGN.category, "groupuscule");
  });
