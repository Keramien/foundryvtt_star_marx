...
export class StarMarxActor extends Actor {
  ...
  async _preUpdate(changed, options, user) {
    ...
    // Soyouz equivalent: health.max depends on MUR DE FER (score + bonus +
    // avaries). Shift health.value by the same delta so wound count persists.
    if (this.type === "soyouz") {
      const watched = [
      ...
      if (!watched.some(p => foundry.utils.hasProperty(changed, p))) return;
